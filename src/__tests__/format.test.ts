...
describe("buildQS", () => {
  ...
  it("stringifies numbers and booleans", () =>
    expect(buildQS({ page: 1, db_only: true })).toBe("?page=1&db_only=true"));
  it("treats empty array as absent", () => expect(buildQS({ tags: [] })).toBe(""));
});

describe("summarizeDocs", () => {
  it("strips content from paginated results", () =>
    expect(summarizeDocs({ count: 1, results: [{ id: 1, title: "D", content: "x" }] })).toEqual({
      count: 1,
      results: [{ id: 1, title: "D" }],
    }));
