...
function collect(
  reg: (s: any, c: InstanceType<typeof PaperlessClient>) => void,
  c: InstanceType<typeof PaperlessClient>,
) {
...
describe("core tools use the injected client", () => {
  ...
  it("download_document calls client.download", async () => {
    const c = new PaperlessClient("https://p.example.com", "user-tok");
    let path = "";
    (c as any).download = async (p: string) => {
      path = p;
      return {
        ok: true,
...
describe("helper tools use the injected client", () => {
  it("get_documents calls client.fetch per id", async () => {
    ...
    (c as any).fetch = async (p: string) => {
      calls.push(p);
      return { id: Number(p.split("/")[3]), content: "x" };
    };
  ...
  it("upload_from_url calls client.upload", async () => {
    const c = new PaperlessClient("https://p.example.com", "user-tok");
    let uploaded = false;
    (c as any).upload = async () => {
      uploaded = true;
      return { ok: true, json: async () => ({ task: "t" }), headers: new Map() };
    };
