...
import { paperlessFetch, fetchAllPages, getDocumentContent, ok, err } from "../paperless.js";
...
export function registerSearchTools(server: McpServer) {
  server.tool(
    ...
    async ({ query, limit }) => {
      try {
        ...
        const results = searchSimilar(queryEmbedding, limit || 10);
        return ok(results);
  ...
  server.tool(
    "sync_embeddings",
    "Sync document embeddings to the local vector database. Indexes all documents for semantic search. Only re-embeds documents whose content has changed.",
    {
    ...
    async ({ force }) => {
      try {
        ...
        const docs = await fetchAllPages<PaperlessDocument>("/api/documents/");
        ...
        for (let i = 0; i < docs.length; i += BATCH_SIZE) {
          ...
          for (const doc of batch) {
            try {
              ...
              if (!content) {
                const detail = await paperlessFetch(`/api/documents/${doc.id}/`) as PaperlessDocument;
